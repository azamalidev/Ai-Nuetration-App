...
function VideoCallUI({
  ...
  const handleSend = () => {
    ...
    setMessages((prev) => [...prev, { sender: userEmail, text: input }]);
    setInput("");
  };
...
>
  {/* Remote participant (fills full screen) */}
      ...
      .map((p) => (
        <ParticipantView
          ...
          style={{
            width: "100%",
            height: "100%",
            objectFit: "cover", // ✅ fills the screen, crops instead of black bars
          }}
        />
      ))
  ) : (
  ...
  {/* Local participant preview (small window bottom-right) */}
  {participants
    .filter((p) => p.isLocalParticipant)
    .map((p) => (
      <div
      ...
      >
        <ParticipantView
          participant={p}    showParticipantDetails={false}
          style={{
            width: "100%",
            height: "100%",
            objectFit: "cover",
      ...
      >
        ...
        <ControlButton active={showParticipants} onClick={toggleParticipants}>
          <Users />
        </ControlButton>
...
// ---------------- CONTROL BUTTON ----------------
function ControlButton({ children, onClick, active, destructive }) {
  return (
    <button
      onClick={onClick}
      style={{
